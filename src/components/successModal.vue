<script setup lang="ts">
 ...
 let status = computed<boolean>(()=>{ return store.get_successModal })
 let message = computed<string>(()=>{ return store.get_success_message})

 function closeOnOutsideClick(event: MouseEvent) {
    const modalContent = document.getElementById("alert-modal-content");

    if (modalContent && !modalContent.contains(event.target as Node)) {
      store.change_success_status("")
    }
 }

 function Close(){ store.change_success_status("") }
</script>
  ...
  <AnimatePresence :initial="false">
    ...
    >
      ...
      <motion.section
        ...
        :class="useThemeComposable() ? 'bg-Dark border-green-500 text-green-500'
        :'bg-white shadow-6xl text-green-500'"
      >
